...
Server::Server(int port){ //Createasocket
...
void Server::startChat(){
    ...
    while(true) {
        ...
        else if (strcmp("open calendar", buffer) == 0)
        {
            send(clientSocket, "done! ", strlen(buffer), 0);
            int status = std::system(Server::open_calendar);
        }
        else if (strcmp("open facebook", buffer) == 0)
        {
            send(clientSocket, "done! ", strlen(buffer), 0);
            int status = std::system(Server::open_facebook_command);
        }
        else if (strcmp("battery", buffer) == 0)
        {
            send(clientSocket, "done! ", strlen(buffer), 0);
            int status = std::system(Server::battery);
        }
        else if (strcmp("logs", buffer) == 0)
        {
            send(clientSocket, "done! ", strlen(buffer), 0);
            int status = std::system(Server::logs);
        }
        else if (strcmp("disks", buffer) == 0)
        {
            send(clientSocket, "done! ", strlen(buffer), 0);
            int status = std::system(Server::disks);
        }
        else if (strcmp("control center", buffer) == 0)
        {
            send(clientSocket, "done! ", strlen(buffer), 0);
            int status = std::system(Server::control_center);
        }
        else if (strcmp("todo list", buffer) == 0)
        {
            send(clientSocket, "done! ", strlen(buffer), 0);
            int status = std::system(Server::todo);
        }
        else if (strcmp("system monitor", buffer) == 0)
        {
            send(clientSocket, "done! ", strlen(buffer), 0);
            int status = std::system(Server::system_monitor);
        }
